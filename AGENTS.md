...
| Task | Command |
|---|---|
| add papers (`download` skill) | `zc.py add <links...>` — everything in one go; pauses only when the collection is uncertain (or, with `ZC_LLM=agent`, for the ADJUDICATE block) |
