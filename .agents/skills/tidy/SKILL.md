---
name: tidy
description: Organize items the user dropped into Zotero by hand (no status tag yet) — metadata, title format, URL, tags, collection. Use when the user says they dragged PDFs into Zotero or asks to tidy / organize the library.
---
...
2. Handle the output exactly like `download`: **PAUSE** rows -> the printed `python3 zl.py tidy --only <key> ...` line
   with `--collection "..."` (collection undecided, decide from the printed abstract) and / or `--confirm a,b` /
   `--confirm none` (ADJUDICATE block when `ZC_LLM=agent`; confirm only candidates the paper's own method or experiments
   satisfy); candidates -> report them; obviously wrong tags -> `python3 zl.py untag`.
