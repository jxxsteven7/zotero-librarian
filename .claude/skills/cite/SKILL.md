# /cite — citation neighbourhood of a paper, cross-checked with the library
...
```
python3 zc.py cite <key|arXiv|DOI|link> [--top 15]
python3 zc.py cite --library [--top 20]        # ~1 request per library paper, cached in cache/s2/
```

Relay the printed lists; point out which references the user already has and which highly cited ones are missing.
Add with `/download <arXiv id or DOI>`. Without `S2_API_KEY` in `.env` the API allows about one request per second;
`--library` on a large library takes a few minutes the first time.
