#!/usr/bin/env python3
"""第一轮提案（approval mode）。数据即提案：批准后 apply 脚本直接读 PROPOSAL / RENAMES。
...
P = {
 ...
 "HEPC5DYG": ([DM], ["policy-learning","rl"], ["gripper","dual-arm","bimanual"], ["diffusion"], [], ["point-cloud","tactile"], "to-read", "软夹爪+压阻触觉, 扩散策略→仿真 RL 微调; vision 是否作为策略输入不确定; 需改名"),
 "UHXQKX93": ([MISC], ["foundation-model"], [], ["transformer"], [], ["vision"], "to-read", "第一人称视频→手部/相机运动重建, 非操作策略; 用户原放 Dex-Hand, 按规则字面归 Misc, 请定; 需改名"),
 "6Z789W2Q": ([DM], ["foundation-model"], ["gripper","single-arm","dual-arm"], [], [], ["vision","language"], "to-read", "VLM 智能体输出离散语义动作, 非 VLA; Franka / AgileX; 需改名"),
 "G4N8QAK5": ([DM], ["vla","rl"], ["single-arm","dual-arm","bimanual","dex-hand","gripper"], ["action-chunking","flow-matching","hil"], ["pi0.5"], ["vision","language"], "to-read", "UR5e 夹爪 / UR5e+LinkerHand L20 / Franka / 双 UR5e; 干预引导; 需改名"),
 ...
 "FGJSCC74": ([DM], ["rl","policy-learning"], ["dex-hand","bimanual"], [], [], [], "to-read", "仿真, Inspire/Shadow/MANO 浮动手无臂 → 不贴 dual-arm"),
 "NSQ8FPP4": ([DM], ["policy-learning"], ["gripper","single-arm","dual-arm","bimanual"], ["diffusion","action-chunking"], [], ["vision","tactile"], "to-read", "Flexiv Rizon + GelSight Mini/MCTac; TactAR 遥操"),
 ...
 "UCJ6JZJN": ([DM], ["policy-learning"], ["gripper","dual-arm","bimanual"], ["diffusion"], [], ["point-cloud","tactile"], "to-read", "鳍形软夹爪+触觉阵列, 3D 统一表征"),
 ...
 "B6GDMCM5": ([DM], ["rl"], ["dex-hand","single-arm","dual-arm","bimanual"], [], [], [], "to-read", "仿真 Allegro+Kuka, 单/双臂系统"),
 ...
 "AP8LAAEQ": ([DM], ["vla","rl"], ["dex-hand","dual-arm","bimanual"], ["flow-matching"], ["pi0"], ["vision","language","tactile"], "to-read", "Sharpa 双臂, 10 指尖 6D 力; π0 骨干 + MoDE; RL 原子技能辅助遥操"),
 ...
 "BZVNNFPP": ([DM], ["vla"], ["dex-hand","dual-arm","bimanual"], ["flow-matching","transformer","action-chunking"], [], ["vision","language"], "to-read", "架构类 GR00T N1 但非微调 → 不贴 base; Galaxea R1Pro+Sharpa, Unitree G1 三指"),
 "MWAF9GHZ": ([DM], ["vla","rl"], ["gripper","dual-arm","bimanual"], ["action-chunking","hil"], [], ["vision","language"], "to-read", "1 条批注; 冻结 π0.6 → 提议 base:pi0.6; PI 双臂夹爪"),
 "AFZQFYKE": ([DM], ["vla"], ["gripper","dual-arm","bimanual"], ["flow-matching","action-chunking","latent-cot"], [], ["vision","language"], "to-read", "TRI LBM; 双 Franka; 显式 CoT 条件是实验变量之一"),
 ...
 "N84MPX5T": ([DM], ["vla","rl"], ["gripper","dual-arm","bimanual"], ["flow-matching","action-chunking","hil"], ["pi0.5"], ["vision","language"], "to-read", "RECAP; π0.6 源自 π0.5 故贴 base:pi0.5; 静态双臂+移动"),
 ...
 "N7NPRGW8": ([DM], [], ["dex-hand","dual-arm","bimanual"], [], [], ["vision","tactile"], "to-read", "双 UR7e + Sharpa Wave, Quest 3 → 提议 method:teleop"),
 "XDTPUAGX": ([DM], ["rl"], ["dex-hand"], [], [], [], "to-read", "仿真, Allegro/MANO/Sharpa Wave 各训一策略"),
 "6VBEISKD": ([DM], [], ["dex-hand","single-arm","dual-arm","bimanual"], [], [], [], "to-read", "MuJoCo 基准 Franka+Allegro, 含双手任务; 基线 π0/GR00T 不贴 base → 提议 type:benchmark"),
 "DEBKIIKT": ([DM], [], ["gripper","dual-arm","bimanual"], [], [], [], "to-read", "ARX X5 双臂仿真+实机基准 → 提议 type:benchmark"),
 ...
 "IHBAME45": ([DM], ["vla"], ["single-arm","dual-arm","gripper","dex-hand"], ["latent-cot","flow-matching","transformer","action-chunking"], [], ["vision","language"], "read", "27 条批注; Janus-Pro/DeepSeek-LLM 1.5B 骨干; 10 实机任务含单/双臂/移动/灵巧手(正文已核)"),
 "SKVYZF8Z": ([DM], ["vla","world-model"], ["dual-arm","gripper","dex-hand"], ["latent-cot","transformer","flow-matching"], [], ["vision","language"], "to-read", "Galaxea R1 Lite / Tianji Marvin / +WUJI; 辅助动作条件世界模型"),
 ...
 "493QZDBC": ([DM], ["vla"], ["gripper","dual-arm","bimanual"], ["flow-matching","action-chunking","latent-cot"], [], ["vision","language"], "to-read", "π 系列新模型本身, 不贴 base; 子任务语言/子目标图像预测计入 latent-cot; UR5e 双臂/BiPi/移动"),
 "FD8M3XF9": ([DM], ["rl","policy-learning"], ["gripper","dex-hand","single-arm","dual-arm"], ["diffusion","action-chunking"], [], ["vision","point-cloud"], "to-read", "UR5 / Franka+LeapHand / xArm; 实机 RL; 无 hil"),
 "FBQP3MT6": ([DM], ["vla"], ["dex-hand","dual-arm","bimanual"], ["flow-matching","transformer","action-chunking"], [], ["vision","language","tactile"], "to-read", "❗; Dexmate Vega-1 + 2×Sharpa Wave; Qwen3VL-2B 骨干"),
 "6Z4ZT39B": ([DM], ["policy-learning"], ["dual-arm","bimanual","gripper"], ["action-chunking","transformer"], [], ["vision"], "read", "13 条批注; ALOHA 硬件, 无语言"),
 ...
 "Q2IUL8SA": ([DM], ["policy-learning"], ["dex-hand","dual-arm","bimanual"], ["transformer"], [], ["vision","tactile"], "to-read", "❗; 2×Sharpa Wave, ZED 立体+腕部视角"),
 "9NRZ28N7": ([DM], ["rl"], ["dex-hand","single-arm"], [], [], [], "to-read", "❗; LEAP/WUJI; 单示范重定向 + 残差 RL"),
 "L23X3VK2": ([DM], ["world-model","policy-learning"], ["dex-hand","dual-arm","bimanual"], ["flow-matching","action-chunking","transformer"], [], ["vision","tactile"], "to-read", "❗; 双臂双 Sharpa Wave + 触觉; language 是否为输入不确定"),
