"""Subcommands of zc.py. Rules live in CLAUDE.md, the taxonomy in taxonomy.toml. (On Windows, `python` instead of `python3`.)"""
import argparse, sys

from . import config   # noqa: F401 — forces UTF-8 stdout before anything prints (Windows pipes)

USAGE = """python3 zc.py <command> ...        Zotero library maintenance (rules: CLAUDE.md, taxonomy: taxonomy.toml)
...
Adding papers (the /download skill)
  add <link|PDF path>...   fetch -> classify (rules + local LLM) -> save via Zotero desktop -> verify on the server -> commit log -> report
        ...
        [--url URL] [--short NAME] [--name TITLE] [--wait SECONDS (150)] [--no-commit] [--dry-run]
...
Organizing what is already in Zotero
  tidy [--only K1,K2] [--collection X] [--dry-run] [--wait S] [--no-commit]
...
Library and batch
  ...
  setup                    health check (Python / .env / data dir / taxonomy / pdftotext / network / Zotero / API key / LLM)
"""
