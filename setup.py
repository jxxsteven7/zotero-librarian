...
"""新机器体检（Ubuntu / macOS / Windows 通用，只用标准库，不改任何东西）：
    ...
    python setup.py          # Windows 一般叫 python；Linux/macOS 叫 python3（./setup.sh 会自己挑）
...
try:
    https("https://export.arxiv.org/api/query?search_query=id:2410.24164&max_results=1"); report(True, "HTTPS 出网正常（arXiv API）")
...
print()
if problems:
    print(f"还差 {len(problems)} 项（见上面 ✗）。修好后再跑一次 {PY} setup.py"); sys.exit(1)
